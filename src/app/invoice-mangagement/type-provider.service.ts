...
export class TypeProviderService {
  ...
  getAllPurchase(): Observable<any>{
    return this.http.get<PurchaseOrder>(this.API_URL).pipe(
      map(result => result)
    )
  };
  ...
  PostPurchase(Data: PurchaseOrder){
    ...
    return this.http.post<PurchaseOrder>(this.API_URL,JSON.parse(JSON.stringify(Data)),this.httpOptions);
  }

  postInvoice(Data:invoiceOrder){
    console.log(this.API_URL_Invoice, Data);
    return this.http.post<PurchaseOrder>(this.API_URL_Invoice,JSON.parse(JSON.stringify(Data)),this.httpOptions);
  }
